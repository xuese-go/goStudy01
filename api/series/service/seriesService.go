...
新增
*/
func Save(mod structs.SeriesStructs) resp.ResponeStruct {
	dba := db.Db
	tx := dba.Begin()
...
修改
*/
func Update(mod structs.SeriesStructs) resp.ResponeStruct {
	dba := db.Db
	tx := dba.Begin()
	var u structs.SeriesStructs
...
分页查询
*/
func Page(pageNum int, pageSize int, mod structs.SeriesStructs) resp.ResponeStruct {
	//为了不影响后边的操作  所以需要使用新的变量
	dba := db.Db
	us := make([]structs.SeriesStructs, 0)
